...
<div style="display: flex; align-items: center; gap: 15px;">
    ...
    <div style="flex-grow: 1; text-align: center; margin-top: -70px;">
        <h4 style="margin: 5px 0; font-weight: bold;">KEMENTERIAN PENDIDIKAN, KEBUDAYAAN, RISET, DAN TEKNOLOGI</h4>
        <h5 style="margin: 0; font-size: 16px;">POLITEKNIK NEGERI BANYUWANGI</h5>
                ...
                @foreach ($pendapatan as $item)
                    <li>
                        <span style="display: inline-block; min-width: 325px;">
                            {{ $item->jenisPendapatan->nama_jenis_pendapatan }}
                        </span>
                        : Rp{{ number_format($item->nilai_netto, 0, ',', '.') }}
                    </li>
                    @php $totalPenerimaan += $item->nilai_netto; @endphp
                @endforeach
            </ul>
        </td>
